package = github.com/chienfuchen32/mongodb_exporter
TAG := $(shell git tag | sort -r | head -n 1)

test:
	go test github.com/chienfuchen32/mongodb_exporter/collector -cover -coverprofile=collector_coverage.out -short
	go tool cover -func=collector_coverage.out
	go test github.com/chienfuchen32/mongodb_exporter/shared -cover -coverprofile=shared_coverage.out -short
